...
# Ex: rake notice:trigger_sep_denial_notice_to_employees["name_of_the_file"]
namespace :notice do
  desc "Trigger SEP denial notice to employees"
  task :trigger_sep_denial_notice_to_employees, [:file_name] => :environment do |task, args|
    ...
    CSV.foreach("#{file_path}", headers: true) do |row|
      ...
      if employee_role && employee_role.census_employee
        begin
          ...
          puts "Notice Triggered for hbx_id #{row['hbx_id']}" unless Rails.env.test?
        rescue Exception => e
          puts "Error triggering #{event_name} notice to hbx_id #{row['hbx_id']} due to #{e}" unless Rails.env.test?
        end
      end
    end
  end
