...
RSpec.describe ShopEmployerNotices::EmployeeTerminatingCoverage, :dbclean => :after_each do
  ...
  let!(:hbx_enrollment) do
    FactoryBot.create(:hbx_enrollment, benefit_group_assignment: benefit_group_assignment, household: family.active_household, effective_on: TimeKeeper.date_of_record.beginning_of_month + 2.month, plan: renewal_plan,
                                       aasm_state: 'coverage_termination_pending', employee_role_id: employee_role.id, terminated_on: TimeKeeper.date_of_record.end_of_month)
  end
