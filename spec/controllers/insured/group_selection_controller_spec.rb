...
RSpec.describe Insured::GroupSelectionController, :type => :controller, dbclean: :after_each do
  ...
  let!(:blue_collar_census_employees) do
    ees = FactoryGirl.build_list(:census_employee, 1, employer_profile: benefit_sponsorship.profile, benefit_sponsorship: benefit_sponsorship)
    ees.each do |ee|
  ...
  let!(:hbx_enrollment) do
    FactoryGirl.create(:hbx_enrollment, household: family.active_household,
  ...
  let(:benefit_package) do
    FactoryGirl.build(:benefit_package,
  ...
  context 'family with active enrollment and sep' do
    ...
    context "active household nil" do
      before do
        allow(family).to receive(:active_household).and_return(nil)
        it "should show a flash error message" do
          expect(flash[:warning]).to eq("User has not created a family. Please contact customer service at #{Settings.contact_center.phone_number}")
        end
      end
