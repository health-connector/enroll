...
RSpec.describe Employers::CensusEmployeesController, dbclean: :after_each do
  ...
  describe "GET edit" do
    ...
    context 'without permissions' do
      ...
      it 'return error' do
        sign_in(user)
        get :edit, params: { :id => census_employee.id, :employer_profile_id => employer_profile_id, census_employee: {} }
  ...
  describe "GET rehire" do
    ...
    context "with rehiring_date" do
      it "should be error when has no new_family" do
        ...
        allow(EmployerProfile).to receive(:find).with(employer_profile_id).and_return(employer_profile)
        allow(CensusEmployee).to receive(:find).and_return(census_employee)
        get :rehire, params: { :census_employee_id => census_employee.id, :employer_profile_id => employer_profile_id, rehiring_date: (TimeKeeper.date_of_record + 30.days).strftime('%m/%d/%Y') }, :format => :js
        expect(response).to have_http_status(:success)
