...
RSpec.describe Exchanges::HbxProfilesController, dbclean: :after_each do
  ...
  let(:hbx_staff_role) { HbxStaffRole.create!(person: person, permission_id: hbx_staff_permission.id, subrole: "hbx_staff", hbx_profile_id: hbx_profile.id) }
  ...
  describe "Action # employer_datatable" do
      before :each do
        user.last_portal_visited = "www.google.com"
        sign_in(user)
      end
  ...
  describe "#view_the_configuration_tab?" do
    it "should render the config index for a super admin" do
      allow(hbx_staff_role).to receive(:permission).and_return(super_admin_permission)
      sign_in(user)
      get :configuration
      expect(response).to have_http_status(:success)
  ...
  describe "GET family index" do
    ...
    it "redirects if not csr or hbx_staff 'families index' template for hbx_staff" do
      allow(user).to receive(:has_role?).with(:csr).and_return false
      allow(user).to receive(:has_role?).with(:hbx_staff).and_return false
      get :family_index
  ...
  describe "POST" do
    ...
    it "sends timekeeper a date" do
      ...
      allow(hbx_staff_permission).to receive(:can_submit_time_travel_request).and_return(true)
    ...
    it "update setting fails because not updateable" do
      allow(hbx_staff_permission).to receive(:modify_admin_tabs).and_return(false)
      sign_in(user)
  ...
  describe "GET edit_dob_ssn" do
    after :each do
      hbx_staff_permission.update_attributes!(can_update_ssn: false)
    end

    it "should render the edit_dob_ssn partial for logged in users with an admin role" do
      hbx_staff_permission.update_attributes!(can_update_ssn: true)
      sign_in(user)
  ...
  describe "POST reinstate_enrollment", :dbclean => :around_each do
    ...
    after :each do
      hbx_staff_permission.update_attributes!(can_reinstate_enrollment: false)
    end

    it "should redirect to root path" do
      hbx_staff_permission.update_attributes!(can_reinstate_enrollment: true)
  ...
  describe "POST update_enrollment_terminated_on_date", :dbclean => :around_each do
    ...
    context "shop enrollment" do
      context "with valid params" do
        ...
        it "should render template " do
          xhr :post, :update_enrollment_terminated_on_date, enrollment_id: enrollment.id.to_s, family_actions_id: family.id, new_termination_date: TimeKeeper.date_of_record.to_s, format: :js
          expect(response).to have_http_status(:redirect)
          expect(response).to redirect_to(exchanges_hbx_profiles_root_path)
        end
        ...
        context "enrollment that already terminated with future date" do
          context "with new future termination date" do
            it "should update enrollment with new end date and notify enrollment" do
              expect_any_instance_of(HbxEnrollment).to receive(:notify).with("acapi.info.events.hbx_enrollment.terminated", {:reply_to=>glue_event_queue_name, "hbx_enrollment_id" => enrollment.hbx_id, "enrollment_action_uri" => "urn:openhbx:terms:v1:enrollment#terminate_enrollment", "is_trading_partner_publishable" => false})
              xhr :post, :update_enrollment_terminated_on_date, enrollment_id: enrollment.id.to_s, family_actions_id: family.id, new_termination_date: (TimeKeeper.date_of_record + 1.day).to_s, format: :js
              enrollment.reload
    ...
    context "with invalid params" do
      it "should redirect to root path" do
        xhr :post, :update_enrollment_terminated_on_date, enrollment_id: '', family_actions_id: '', new_termination_date: '', format: :js
        expect(response).to have_http_status(:redirect)
        expect(response).to redirect_to(exchanges_hbx_profiles_root_path)
      end
    end
  ...
  describe "extend open enrollment" do
    ...
    before :each do
      hbx_staff_permission.update_attributes!(can_extend_open_enrollment: true)
      allow(::BenefitSponsors::BenefitSponsorships::BenefitSponsorship).to receive(:find).and_return(benefit_sponsorship)
      sign_in(user)
    end

    after :each do
      hbx_staff_permission.update_attributes!(can_extend_open_enrollment: false)
    end
  ...
  describe "GET edit_fein" do
    context "of an hbx super admin clicks Change FEIN" do
      ...
      before :each do
        hbx_staff_permission.update_attributes!(can_change_fein: true)
      end

      after :each do
        hbx_staff_permission.update_attributes!(can_change_fein: false)
      end
