...
describe 'import conversion employer/employee details ', :dbclean => :after_each do
  ...
  context 'conversion_import:employers' do
    ...
    it 'should generate csv with given headers' do
      expected_csv_headers = %w[
        Action FEIN Doing\ Business\ As Legal\ Name Issuer\ Assigned\ Employer\ ID SIC\ code Physical\ Address\ 1 Physical\ Address\ 2 City County County\ FIPS\ code State Zip Mailing\ Address\ 1 Mailing\ Address\ 2
  ...
  context 'conversion_import:employees' do
    ...
    it 'should generate csv with given headers' do
      headers = %w[
        Action Type\ Of\ Enrollment Market Sponsor\ Name FEIN Issuer\ Assigned\ Employer\ ID HIRED\ ON Benefit\ Begin\ Date Plan\ Name HIOS\ ID Premium\ Total
        Employer\ Contribution Employee\ Responsible\ Amount Subscriber\ SSN Subscriber\ DOB Subscriber\ Gender Subscriber\ First\ Name Subscriber\ Middle\ Name
        Subscriber\ Last\ Name Subscriber\ Email Subscriber\ Phone Subscriber\ Address\ 1 Subscriber\ Address\ 2 Subscriber\ City Subscriber\ State Subscriber\ Zip SELF
      ]
      dep_headers = []
      7.times do |i|
