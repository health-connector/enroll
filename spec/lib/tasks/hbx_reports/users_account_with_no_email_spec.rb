...
describe 'user account with no email address', :dbclean => :after_each do
  describe 'report:user_account:with_no_email_address' do
    ...
    it 'should generate csv report with user and person information' do
      ...
      result =  [["username", "user_first_name", "user_last_name", "user_roles", "person_hbx_id", "person_home_email", "person_work_email", "user_created_at"],
                 [user.oim_id.to_s, "John", person.last_name, "[\"consumer\"]", person.hbx_id, person.emails.first.address, "", person.user.created_at.to_s]]
