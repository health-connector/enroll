  ...
  RSpec.describe "hbx_admin/_edit_aptc_csr", :dbclean => :after_each do
    ...
    context "without enrollment" do
      it "Should display the Editing APTC/CSR text" do
        ...
        expect(rendered).to match(%r{Editing APTC / CSR for:})
      end
      ...
      it "Should display the Household Information" do
        ...
        expect(rendered).to match(%r{APTC Amount / Percent Ratio}i)
      end
    end
    ...
    context "with enrollment" do
      ...
      let!(:hbx_with_aptc_1) do
        FactoryBot.create(:hbx_enrollment, household: household, is_active: true, aasm_state: 'coverage_selected', changing: false, effective_on: (TimeKeeper.date_of_record.beginning_of_month - 40.days), applied_aptc_amount: 100)
      end
      let!(:hbx_with_aptc_2) do
        FactoryBot.create(:hbx_enrollment, household: household, is_active: true, aasm_state: 'coverage_selected', changing: false, effective_on: (TimeKeeper.date_of_record.beginning_of_month + 10.days), applied_aptc_amount: 210)
      end
