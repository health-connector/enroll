...
if ExchangeTestingConfigurationHelper.individual_market_is_enabled?
  RSpec.describe HbxAdminHelper, :type => :helper do
    ...
    let(:hbx_inactive)   do
      FactoryBot.create(:hbx_enrollment, household: household, is_active: false, aasm_state: 'coverage_terminated', changing: false, effective_on: (TimeKeeper.date_of_record.beginning_of_month - 40.days), applied_aptc_amount: 100)
    end
    let(:hbx_without_aptc)   do
      FactoryBot.create(:hbx_enrollment, household: household, is_active: true, aasm_state: 'coverage_selected', changing: false, effective_on: (TimeKeeper.date_of_record.beginning_of_month - 40.days), applied_aptc_amount: 0)
    end
