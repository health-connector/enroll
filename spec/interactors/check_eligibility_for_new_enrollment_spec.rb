...
describe CheckEligibilityForNewEnrollment, :dbclean => :after_each do
  ...
  context 'CheckEligibilityForNewEnrollment' do
    ...
    it 'when ce passed' do
      allow(ce).to receive(:new_hire_enrollment_period).and_return(((TimeKeeper.date_of_record + 1.day)..(TimeKeeper.date_of_record + 10.day)))
      context = described_class.call(census_employee: ce, params: {is_waiving: true, family_member_ids: []})
      expect(context.failure?).to eq true
      expect(context.message).to eq "You're not yet eligible under your employer-sponsored benefits. Please return on #{ce.new_hire_enrollment_period.begin.strftime('%m/%d/%Y')} to enroll for coverage."
    end
    ...
    it 'when is_waiving is blank and family members are nil' do
      allow(ce).to receive(:new_hire_enrollment_period).and_return(((TimeKeeper.date_of_record - 1.day)..(TimeKeeper.date_of_record + 10.day)))
      context = described_class.call(census_employee: ce, params: {is_waiving: nil, family_member_ids: []})
      expect(context.failure?).to eq true
