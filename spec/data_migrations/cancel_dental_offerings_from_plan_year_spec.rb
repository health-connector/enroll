...
describe CancelDentalOfferingsFromPlanYear, dbclean: :after_each do
  skip "ToDo rake was never updated to new model, check if we can remove it" do
    ...
    describe "changing plan year's state", dbclean: :after_each do
      ...
      around do |example|
        ClimateControl.modify fein: organization.fein, aasm_state: organization.employer_profile.renewing_plan_year.aasm_state, benefit_group_id: organization.employer_profile.renewing_plan_year.benefit_groups.first.id do
          example.run
        end
      end
