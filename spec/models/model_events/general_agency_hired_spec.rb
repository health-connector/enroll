...
describe 'ModelEvents::GeneralAgencyHired', dbclean: :around_each  do
  let(:model_event)  { "general_agency_hired" }
  let(:notice_event) { "general_agency_hired" }
  ...
  let!(:general_agency_account) { FactoryGirl.create :general_agency_account, aasm_state: 'active',employer_profile: employer_profile, general_agency_profile_id: model_instance.id, end_on: TimeKeeper.date_of_record}
  let!(:general_agency_staff_role) { FactoryGirl.create(:general_agency_staff_role, general_agency_profile_id: model_instance.id, :aasm_state => 'active')}
  ...
  describe "ModelEvent" do
    context "when general_agency is hired" do
      it "should trigger model event" do
        ...
        model_instance.trigger_model_event(:general_agency_hired)
      end
    end
  end

  describe "NoticeTrigger" do
    context "when event_key is general_agency_hired" do
      subject { Observers::NoticeObserver.new }
      it "should trigger notice event" do
        expect(subject.notifier).to receive(:notify) do |event_name, payload|
          expect(event_name).to eq "acapi.info.events.general_agency.general_agency_hired"
          expect(payload[:event_object_kind]).to eq 'EmployerProfile'
          expect(payload[:event_object_id]).to eq employer_profile.id.to_s
        end
        subject.deliver(recipient: model_instance, event_object: employer_profile, notice_event: "general_agency_hired")
      end
    end
  end
  ...
  describe "NoticeBuilder" do
    ...
    it "should return general_agency assignment date" do
      expect(merge_model.assignment_date).to eq general_agency_account.start_on.strftime('%m/%d/%Y')
    end
