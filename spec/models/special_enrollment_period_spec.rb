...
RSpec.describe SpecialEnrollmentPeriod, :type => :model, :dbclean => :after_each do
  ...
  let(:shop_lost_insurance_qle)       do
    QualifyingLifeEventKind.create(
  ...
  context "a new instance" do
    ...
    context "with invalid next_poss_effective_date" do
      let(:param_with_valid_next_poss_effective_date) do
        {
          family: family,
          qualifying_life_event_kind: ivl_qle,
          effective_on_kind: "first_of_next_month",
    ...
    context "with invalid optional_effective_on" do
      let(:param_with_valid_optional_effective_on) do
        {
          family: family,
          qualifying_life_event_kind: ivl_qle,
          effective_on_kind: "first_of_next_month",
  ...
  context "is reporting a qle before the employer plan start_date" do
    ...
    let(:census_employee) { FactoryBot.create(:census_employee, first_name: 'John', last_name: 'Smith', dob: '1966-10-10'.to_date, ssn: '123456789', hired_on: Date.new(TimeKeeper.date_of_record.year, 0o3, 14)) }
  ...
  context "is reporting a qle before the employer plan start_date and having an expired plan year" do
    ...
    let(:census_employee) do
      FactoryBot.create :census_employee, employer_profile: organization.employer_profile, dob: TimeKeeper.date_of_record - 30.years,
