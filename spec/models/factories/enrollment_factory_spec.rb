...
describe Factories::EnrollmentFactory, "starting with unlinked employee_family and employee_role", :dbclean => :after_each do
  ...
  let(:benefit_group_assignment) {FactoryBot.build(:benefit_group_assignment, benefit_package: benefit_package)}
  ...
  let(:employee_role) {
    FactoryBot.build(:employee_role, employer_profile: employer_profile)
  }
