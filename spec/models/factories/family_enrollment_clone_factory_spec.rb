...
RSpec.describe Factories::FamilyEnrollmentCloneFactory, :type => :model, dbclean: :after_each do
  ...
  let!(:active_benefit_group_assignment) { FactoryBot.build(:benefit_group_assignment, benefit_package: sponsored_benefit_package, start_on: predecessor_application.start_on, end_on: predecessor_application.end_on, benefit_group_id: nil)}
  let!(:renewal_benefit_group_assignment) { FactoryBot.build(:benefit_group_assignment, start_on: renewal_application.start_on, end_on: renewal_application.end_on, benefit_package: benefit_package, benefit_group_id: nil)}
  let!(:ce) { FactoryBot.create :census_employee, :owner, employer_profile: employer_profile, dob: Date.new((coverage_terminated_on.year - 30), 9,8),  employee_role_id: employee_role.id ,benefit_group_assignments:[active_benefit_group_assignment, renewal_benefit_group_assignment]}
