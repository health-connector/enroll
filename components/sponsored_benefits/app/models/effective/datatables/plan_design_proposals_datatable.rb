    ...
    module Datatables
      class PlanDesignProposalsDatatable < ::Effective::MongoidDatatable
        datatable do
          ...
          table_column :effective_date, :label => 'Effective Date', :proc => proc { |row| proposal_sponsorship(row).initial_enrollment_period.begin.strftime("%m/%d/%Y") }, :sortable => true, :filter => false
