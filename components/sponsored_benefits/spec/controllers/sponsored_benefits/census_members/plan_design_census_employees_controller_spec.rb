      ...
      context "with valid params" do
        ...
        context "when dependent deleted" do
          ...
          it "should drop dependents" do
            expect(census_employee.census_dependents.size).to eq 2
            put :update, xhr: true,
                         params: {plan_design_proposal_id: plan_design_proposal.id, :id => census_employee.to_param, :census_members_plan_design_census_employee => new_attributes.merge(census_dependents_attributes: delete_dependents_attributes)}
            census_employee.reload
