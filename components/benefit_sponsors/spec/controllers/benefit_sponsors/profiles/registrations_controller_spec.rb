  ...
  RSpec.describe Profiles::RegistrationsController, type: :controller, dbclean: :after_each do
    ...
    shared_examples_for "initialize profile for new" do |profile_type|
      describe "GET show", dbclean: :after_each do
        before do
          sign_in edit_user
          get :show, :agency => send("#{profile_type}_params"), :id => send("#{profile_type}_params")[:id]
        end
    ...
    describe "PUT update", dbclean: :after_each do
      ...
      context "updating profile" do
        ...
        def sanitize_attributes(profile_type)
          employer_profile_attributes[:id] = send(profile_type).profiles.first.id.to_s

          broker_profile_attributes[:id] = send(profile_type).profiles.first.id.to_s
