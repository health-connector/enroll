...
RSpec.describe 'BenefitSponsors::ModelEvents::EmployerSponsoredCobraNoticeToEmployee', :dbclean => :after_each do
  ...
  describe "when employee elected for cobra enrollment" do
    ...
    context "ModelEvent" do
      it "should trigger model event" do
        ...
        model_instance.update_for_cobra(model_instance.coverage_terminated_on + 1.day)
      end
    end

    context "NoticeTrigger" do
      subject { BenefitSponsors::Observers::CensusEmployeeObserver.new }
      let(:model_event) { BenefitSponsors::ModelEvents::ModelEvent.new(:employee_notice_for_employer_sponsored_cobra_enrollments, model_instance, {}) }

      it "should trigger notice event" do
        expect(subject.notifier).to receive(:notify) do |event_name, payload|
          expect(event_name).to eq "acapi.info.events.employee.employee_notice_for_employer_sponsored_cobra_enrollments"
