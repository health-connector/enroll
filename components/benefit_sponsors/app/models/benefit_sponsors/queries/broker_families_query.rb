  ...
  module Queries
    class BrokerFamiliesQuery
      ...
      def employee_person_ids
        @census_employee_ids ||= BenefitSponsors::BenefitSponsorships::BenefitSponsorship
                                 .includes(:census_employees)
                                 .where(:broker_agency_accounts => {"$elemMatch" => {:benefit_sponsors_broker_agency_profile_id => @broker_agency_profile_id, is_active: true}})
                                 .map(&:census_employees)
                                 .flatten
                                 .map(&:_id)
                                 .uniq
