  ...
  module Observers
    class BrokerAgencyAccountObserver
      ...
      def broker_hired?(account, _options = {})
        return false unless !account.persisted? && account.valid? && account.benefit_sponsorship?
      ...
      def notifications_send(_model_instance, new_model_event)
        return unless new_model_event.present? && new_model_event.is_a?(BenefitSponsors::ModelEvents::ModelEvent)

        broker_agency_account = new_model_event.klass_instance
        return unless broker_agency_account.benefit_sponsorship.present?
        ...
        return unless BenefitSponsors::ModelEvents::BrokerAgencyAccount::REGISTERED_EVENTS.include?(new_model_event.event_key)
        ...
        return unless new_model_event.event_key == :broker_fired
