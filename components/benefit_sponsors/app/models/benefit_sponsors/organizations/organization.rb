  ...
  module Organizations
    class Organization
      ...
      def employer_profile
        profiles.where(_type: /.*EmployerProfile$/).first
      end

      def broker_agency_profile
        profiles.where(_type: /.*BrokerAgencyProfile$/).first
      end

      def hbx_profile
        profiles.where(_type: /.*HbxProfile$/).first
      end

      def issuer_profile
        profiles.where(_type: /.*IssuerProfile$/).first
      end

      def is_an_issuer_profile?
        profiles.where(_type: /.*IssuerProfile$/).present?
      end
      ...
      class << self
        ...
        def search_agencies_by_criteria(search_params)
          ...
          if query_params.any?
            broker_agency_profiles.approved_broker_agencies.broker_agencies_by_market_kind(['both', 'shop']).where({ "$and" => build_query_params(search_params) })
          else
            broker_agency_profiles.approved_broker_agencies.broker_agencies_by_market_kind(['both', 'shop'])
          end
        end

        def broker_agencies_with_matching_agency_or_broker(search_params)
          if search_params[:q].present?
            orgs2 = broker_agency_profiles.approved_broker_agencies.broker_agencies_by_market_kind(['both', 'shop']).where({
                                                                                                                             :"profiles._id" => {
        ...
        def build_query_params(search_params)
          ...
          query_params << { :"profiles.working_hours" => working_hours_as_bool(search_params[:working_hours])} unless search_params[:working_hours].blank?
