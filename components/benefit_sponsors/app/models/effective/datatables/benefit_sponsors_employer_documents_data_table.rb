      ...
      datatable do
        ...
        table_column :actions, :width => '50px', :proc => proc { |row|
          ...
          dropdown = [
           ...
           ['Delete', main_app.employers_employer_attestation_delete_attestation_documents_path(row.id), @employer_profile.employer_attestation.editable? && row.submitted? ? 'delete ajax with confirm' : 'disabled',
