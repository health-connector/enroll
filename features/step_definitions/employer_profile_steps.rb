...
Given(/(\w+) is a person$/) do |name|
  ...
  User.create(email: email, password: @pswd, password_confirmation: @pswd, person: person, oim_id: email)
end
...
Given(/(\w+) is a person who has not logged on$/) do |name|
  FactoryBot.create(:person, first_name: name)
end
...
Then(/(\w+) is the staff person for an employer$/) do |name|
  person = Person.where(first_name: name).first
  FactoryBot.create(:employer_staff_role, person: person, benefit_sponsor_employer_profile_id: employer_profile.id)
end
...
Then(/(\w+) is applicant staff person for an existing employer$/) do |name|
  person = Person.where(first_name: name).first
  FactoryBot.create(:employer_staff_role, person: person, employer_profile_id: @employer_profile.id, aasm_state: 'is_applicant')
end

When(/(\w+) accesses the Employer Portal/) do |name|
  Person.where(first_name: name).first
  visit '/'
  portal_class = 'interaction-click-control-employer-portal'
...
Then(/(\w+) cannot remove EmployerStaffRole from John/) do |legal_name|
  Person.where(first_name: legal_name).first
  accept_alert do
...
When(/(\w+) removes EmployerStaffRole from Sarah/) do |staff2|
  Person.where(first_name: staff2).first
  accept_alert do
    find(:xpath, '//*[@id="edit_agency"]/div/div/div[1]/table/tbody/tr[2]/td[6]/a').click
  end
end
