        ...
        if bene_app.effective_period.min == start_on && bene_app.is_renewing?
          ...
          ben_spon.census_employees.active.each do |census|
            ...
            ["health", "dental"].each do |kind|
              enrollments_2018 = enrollment.where(coverage_kind: kind, :"effective_on".lt=> start_on)
              enrollments_2019 = enrollment.where(coverage_kind: kind, :effective_on=> start_on)
              next unless enrollments_2018.present? || enrollments_2019.present?
              ...
              csv << [ben_spon.profile.legal_name,
                      ...
                      enrollments_2018.map{|en| en.try(:hbx_id)}.join(','),
                      enrollments_2018.map{|en| en.try(:coverage_kind)}.join(','),
                      enrollments_2018.map{|en| en.try(:product).try(:hios_id)}.join(','),
                      enrollments_2018.map{|en| en.try(:effective_on)}.join(','),
                      enrollments_2018.map{|en| en.try(:aasm_state)}.join(','),
                      enrollments_2019.map{|en| en.try(:hbx_id)}.join(','),
                      enrollments_2018.map{|en| en.try(:coverage_kind)}.join(','),
                      enrollments_2019.map{|en| en.try(:product).try(:hios_id)}.join(','),
                      enrollments_2019.map{|en| en.try(:effective_on)}.join(','),
                      enrollments_2019.map{|en| en.try(:aasm_state)}.join(','),
                      ]
              end
            end
          end
...
# start_on = Date.new(2019,7,1)
reports = MaNfpReports.new(Date.new(2019,8,1))
