module Forms
  class FamilyMember
    ...
    attr_accessor :id, :family_id, :is_consumer_role, :is_resident_role, :vlp_document_id, :gender, :relationship, :addresses, :no_dc_address, :no_dc_address_reason, :same_with_primary, :is_applying_coverage
    attr_writer :family
    ...
    def consumer_fields_validation
      ...
      return unless @is_consumer_role.to_s == "true" && is_applying_coverage.to_s == "true" #only check this for consumer flow.
      ...
      errors.add(:base, "native american / alaskan native status is required") if @indian_tribe_member.nil?

      errors.add(:tribal_id, "is required when native american / alaskan native is selected") if !tribal_id.present? && @indian_tribe_member

      return unless @is_incarcerated.nil?

      errors.add(:base, "Incarceration status is required")
    end
    ...
    def save
      ...
      if existing_inactive_family_member
        ...
        existing_inactive_family_member.reactivate!(relationship)
      ...
      if existing_person
        family_member = family.relate_new_member(existing_person, relationship)
        if is_consumer_role == "true"
          family_member.family.build_consumer_role(family_member)
        elsif is_resident_role == "true"
      ...
      family_member = family.relate_new_member(person, relationship)
      if is_consumer_role == "true"
    ...
    def assign_person_address(person)
      if same_with_primary == 'true'
        ...
        addresses.each_value do |_, address|
          ...
          if address_attrs["address_1"].blank? && address_attrs["city"].blank?
            current_address.destroy if current_address.present?
            next
          end
          if current_address.present?
            current_address.assign_attributes(address_attrs)
            update_census_dependent(person)
            current_address.save
          else
            person.addresses.create(address_attrs)
          end
        end
      end
      true
    ...
    def relationship_validation
      ...
      relationships[id.to_s] = relationship
      return unless relationships.values.count{|rs| ['spouse', 'life_partner'].include?(rs)} > 1

      errors.add(:base, "can not have multiple spouse or life partner")
    end
  end
end
