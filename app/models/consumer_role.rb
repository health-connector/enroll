...
class ConsumerRole
  ...
  def setup_lawful_determination_instance
    self.lawful_presence_determination = LawfulPresenceDetermination.new unless lawful_presence_determination.present?
  end
  ...
  def check_for_critical_changes(person_params, family)
    redetermine_verification!(verification_attr) if person_params.select{|k,_v| VERIFICATION_SENSITIVE_ATTR.include?(k) }.any?{|field,_v| sensitive_information_changed(field, person_params)} && family.person_has_an_active_enrollment?(person)
...
# initial trigger check for coverage purchase
  ...
  def revert_lawful_presence(*args)
    lawful_presence_determination.revert!(*args)
  end

  def update_all_verification_types(*_args)
  ...
  def ensure_ssn_validation_status
    self.ssn_validation = "na" if person && person.ssn.blank?
  end
