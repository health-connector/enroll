class LawfulPresenceDetermination
  ...
  def latest_denial_date
    ...
    if verification_outstanding? && responses.present?
  ...
  def record_approval_information(*args)
    ...
    return unless ["ssa", "curam"].include?(approval_information.vlp_authority)
    return unless ivl_role
    return unless ivl_role.person
    return if ivl_role.person.ssn.blank?

    ivl_role.ssn_validation = "valid"
  end
