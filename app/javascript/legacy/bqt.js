...
function loadFunctions() {
  new Promise((resolve, reject) => {
    const form = document.getElementById('new_forms_plan_design_proposal');
    if (form) {
      resolve(true)
    } else {
      reject('Page didnt load')
    }
  })
  .then((data) => {
    if (data) {
...
function pageInit() {
  let kind = fetchBenefitKind();

  if(kind == "dental") {
    let dental_reference_plan_id = $("#dental_reference_plan_id").val();
...
function attachEmployerHealthContributionShowHide() {
  let offering_id = $(this).attr("data-offering-id");
  let option_kind = $(this).attr("data-offering-kind");
...
function fetchCarriers() {
  let active_year = $("#forms_plan_design_proposal_effective_date").val().substr(0,4);
  let selected_carrier_level = $(this).siblings('input').val();
  let plan_design_organization_id = $('#plan_design_organization_id').val();
  let kind = $("#benefits_kind").val();
...
function setSBC(element, plan) {
  let kind = fetchBenefitKind();
  let sbc_link = element.parentElement.getElementsByClassName("sbc-download-checkbox")[0]
...
function downloadPdf(event, element) {
  ...
  let data = buildBenefitGroupParams();
...
function carrierSelected() {
  $('.tab-container').hide();
  let elected_plan_kind = $('.plan-design .nav-tabs li.active input').val();
  let selected_rpids = [];
...
function setMyPlans(element) {
  ...
  let reference_plan_id = element.dataset.planid.replace(/['"]+/g, '');
  let kind = fetchBenefitKind();

  let plan_option_kind = $("#elected_plan_kind").val();
  if(kind == "dental") {
    let plan_option_kind = $("#dental_elected_plan_kind").val();
  }
...
function calcPlanDesignContributions() {
  let effectiveDate = $("#forms_plan_design_proposal_effective_date").val()
  let data = buildBenefitGroupParams();
...
function buildBenefitGroupParams() {
  // let kind = $("#benefit_kind").val();
  let kind = fetchBenefitKind();
  let reference_plan_id = $('#reference_plan_id').val();

  if(kind == "dental") {
    let reference_plan_id = $("#dental_reference_plan_id").val();
  }
  ...
  let premium_pcts = $('.enabled .benefits-fields input.hidden-param').map(function() {
    return $(this).val();
  }).get();
  let is_offered = $('.enabled .benefits-fields .checkbox label > input[type=checkbox]').map(function() {
    return $(this).is(":checked");
  }).get();

  let composite_rating_tier_types = [ 'employee_only', 'family', 'employee_and_spouse', 'employee_and_one_or_more_dependents']
  let relationship_benefit_types = [ 'employee', 'spouse', 'domestic_partner', 'child_under_26', 'child_26_and_over']
  let relation_benefits = {};
...
function setRadioBtn(element) {
  let dotIcons = document.querySelectorAll('.fa-dot-circle');
  let icons = document.querySelectorAll('.fa-circle');
  let iconId = element.target.dataset.tempId;
...
function disableActionButtons() {
  let minimum_employee_contribution = $("#employer_min_employee_contribution").val();
  let minimum_family_contribution = $("#employer_min_family_contribution").val();
  let data = buildBenefitGroupParams();
  if (proposalIsInvalid(data)){
    $(function () {
...
function contributionLevelsAreValid(benefit_group) {
  if ('composite_tier_contributions_attributes' in benefit_group) {
    let contributions = benefit_group['composite_tier_contributions_attributes'];
    return checkContributionLevels(Object.values(contributions));
  } else if ('relationship_benefits_attributes' in benefit_group) {
    let contributions = benefit_group['relationship_benefits_attributes'];
    return checkContributionLevels(Object.values(contributions));
  } else {
    return false;
  }
...
function checkContributionLevels(contributions) {
  let minimum_employee_contribution = $("#employer_min_employee_contribution").val();
  let minimum_family_contribution = $("#employer_min_family_contribution").val();
  let offered_contributions = contributions.filter(function( obj ) {
    return obj.offered;
  });

  let values_to_check = offered_contributions.map(function(obj) {
...
function saveProposal(event) {
  ...
  if (proposalIsInvalid(data) && (effectiveDate !== undefined && effectiveDate.slice(5) !='01-01')) {
    ...
    let url = $("#benefit_groups_url").val();
    $.ajax({
      type: "POST",
      data: data,
      url: url
...
function saveProposalAndCopy(event) {
  ...
  if (proposalIsInvalid(data) && (effectiveDate !== undefined && effectiveDate.slice(5) !='01-01')) {
    ...
    $.ajax({
      ...
      let copy_url = $('#copy_proposal_url').val();
      $.ajax({
        url: copy_url,
        type: 'POST',
        dataType: 'json',
...
function saveProposalAndPublish(event) {
  ...
  if (proposalIsInvalid(data) && (effectiveDate !== undefined && effectiveDate.slice(5) !='01-01')) {
    ...
    $.ajax({
      ...
      let publish_url = $('#publish_proposal_url').val();
      $.ajax({
        url: publish_url,
        type: 'POST',
        success: function(data) {
        ...
        error: function(data) {
          let resp = $.parseJSON(data.responseText);
...
function viewComparisons() {
  let url = $("#plan_comparison_url").val();
...
function sortPlans() {
  let $box = $(this).children('input').first();

  if ($box.is(":checked")) {
    // the name of the box is retrieved using the .attr() method
    // as it is assumed and expected to be immutable
    let group = "input:checkbox[data-search-type='" + $box.data("search-type") + "']";
  ...
  let plans = $('.reference-plan');
  let search_types = {};

  $('.plan-search-option input').each(function(index) {
    let option = $(this);
    let option_is_checked = option.prop('checked');
