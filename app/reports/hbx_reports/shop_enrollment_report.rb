...
class ShopEnrollmentReport < MongoidMigrationTask
  def migrate
    if ENV['purchase_date_start'].blank? && ENV['purchase_date_end'].blank?
      ...
      purchase_date_start = Time.strptime(ENV.fetch('purchase_date_start', nil),'%m/%d/%Y').beginning_of_day
      purchase_date_end = Time.strptime(ENV.fetch('purchase_date_end', nil),'%m/%d/%Y').end_of_day
    end
    ...
    CSV.open(file_name, "w", force_quotes: true) do |csv|
      ...
      enrollment_ids_final.each do |id|
        ...
        begin
          ...
          csv << [employer_id,fein,legal_name,rating_area,plan_year_start,plan_year_state,employer_profile_aasm,eg_id,purchase_time,coverage_start,enrollment_state,subscriber_hbx_id,first_name,last_name,subscriber_ssn,plan_hios_id,is_pvp,
