...
module IndividualMarket
  class HbxAdminController < ApplicationController
    ...
    def edit_aptc_csr
      ...
      @current_year = (params[:year] || Admin::Aptc.years_with_tax_household(@family).max).to_i # Use the selected year or the most recent year for which there is a TH.
    ...
    def calculate_aptc_csr
      ...
      @current_year = (params[:year] || Admin::Aptc.years_with_tax_household(@family).max).to_i
      @person = Person.find(params[:person_id])
      @family = Family.find(params[:family_id])
      @hbxs = @family.active_household.hbx_enrollments.enrolled_and_renewing.by_year(@current_year).by_coverage_kind("health")
      ...
      @plan_premium_for_enrollments = Admin::Aptc.build_plan_premium_hash_for_enrollments(@hbxs)
      @active_tax_household_for_current_year = @family.active_household.latest_active_tax_household_with_year(@current_year)
      @max_aptc = if params[:max_aptc] == "NaN"
                    params[:max_aptc]
                  else
                    format('%.2f', params[:max_aptc])
                  end || @family.active_household.tax_households.tax_household_with_year(@current_year).last.try(:latest_eligibility_determination).try(:max_aptc) || 0
