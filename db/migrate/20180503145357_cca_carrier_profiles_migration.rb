class CcaCarrierProfilesMigration < Mongoid::Migration
  ...
  def self.initialize_new_organization(organization, site)
    json_data = organization.to_json(:except => [:_id, :updated_by_id, :issuer_assigned_id, :versions, :version, :fein, :employer_profile, :broker_agency_profile, :carrier_profile, :hbx_profile, :office_locations, :is_fake_fein, :home_page, :is_active, :updated_by, :documents])
