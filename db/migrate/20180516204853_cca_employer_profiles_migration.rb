...
class CcaEmployerProfilesMigration < Mongoid::Migration
    ...
    say_with_time("Time taken to migrate organizations") do
      old_organizations.batch_size(limit_count).no_timeout.all.each do |old_org|
        ...
        if existing_new_organizations.count == 0
          ...
          json_data = @old_profile.to_json(:except => [:_id,:xml_transmitted_timestamp, :entity_kind, :profile_source, :aasm_state, :registered_on, :contact_method, :employer_attestation, :broker_agency_accounts, :employer_profile_account,
                                                       :plan_years, :updated_by_id, :workflow_state_transitions, :inbox, :documents])
          ...
          raise StandardError unless @benefit_sponsorship.valid?

          BenefitSponsors::BenefitSponsorships::BenefitSponsorship.skip_callback(:save, :after, :notify_on_save)
          @benefit_sponsorship.save!

          raise StandardError unless new_organization.valid?
  ...
  def self.initialize_new_organization(organization, site)
    json_data = organization.to_json(:except => [:_id, :updated_by_id, :issuer_assigned_id, :version, :versions, :employer_profile, :broker_agency_profile, :carrier_profile, :hbx_profile, :office_locations, :is_fake_fein, :is_active, :updated_by,
                                                 :documents])
